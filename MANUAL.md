...
### 1.3 引擎走棋
+ 主界面工具栏中，提供引擎执黑、引擎执红、分析模式等功能。按下对应按钮，界面会调用引擎进行计算；如需取消，再次点击按钮即可。
+ 右侧分析区默认只保留每条候选路线（PV）的最新结果，并随深度原位刷新。每张卡片显示候选排名、推荐首着、红方视角的评估、当前深度和带回合编号的红黑双方着法；底部状态栏集中显示深度、算力、节点数及耗时。
+ 单击候选路线可在棋盘上突出推荐着法，双击可展开或收起完整变例。勾选“历史”可查看本局最多128条思考记录；取消勾选后恢复精简的最新结果。键盘操作时，Enter用于突出当前路线，空格用于展开或收起。    
...
+ 点击引擎菜单，时间设置，可以设置引擎的思考时间及出招延迟等。
    ...
    + 固定深度：引擎搜索到固定的层数之后出招。
    + 搜索广度（MultiPV）：设置引擎同时搜索和显示的候选主变数量，对所有引擎全局生效。默认值为1，推荐设置为1–3；数值越大通常会占用更多处理器资源。不支持MultiPV的引擎会按单主变运行。
